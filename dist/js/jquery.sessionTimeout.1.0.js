...
(function( $ ){
	jQuery.sessionTimeout = function( options ) {
		...
		$('#sessionTimeout-dialog').dialog({
			...
			buttons: {
				...
				"Log Out Now": function() {
					window.location.replace(o.logoutUrl);
				},
			...
			switch(action) {
				case 'start':
					...
					redirTimer = setTimeout(function(){
						window.location.replace(o.redirUrl);
		...
		window.continueSession=false;
		window.sessionActive=function(){if(this.report){continueSession=true;}}
		window.setInterval(function(){if(continueSession){clearTimeout(dialogTimer);dialogTimer=setTimeout(function(){$('#sessionTimeout-dialog').dialog('open');controlRedirTimer('start');},o.warnAfter);/*alert('reset');*/continueSession=false;}},100);
	};
})( jQuery );